...
class Component3DView(posX: Int, posY: Int, width: Int, height: Int) : GuiComponent(posX, posY, width, height) {
    ...
    fun preDraw(mousePos: Vec2d) {
        ...
        run {
            ...
            val LIGHT0_POS = vec(0, 1, 0.1).normalize()//(new Vec3d(0.20000000298023224D, 1D, -0.699999988079071D)).normalize();
            val LIGHT1_POS = vec(0, 1, -0.1).normalize()//(new Vec3d(-0.20000000298023224D, 1D, 0.699999988079071D)).normalize();
