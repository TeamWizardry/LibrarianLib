...
class Component3DView(posX: Int, posY: Int, width: Int, height: Int) : GuiComponent(posX, posY, width, height) {
    ...
    fun calcDrag(mousePos: Vec2d, button: EnumMouseButton?) {
        ...
        if (dragButton == EnumMouseButton.RIGHT) {
            var offset = vec(mousePos.x - dragStart.x, mousePos.y - dragStart.y, 0.0)
            val matrix = Matrix4()
            matrix.rotate(-Math.toRadians(rotZ), vec(0.0, 0.0, 1.0))
            matrix.rotate(-Math.toRadians(rotY), vec(0.0, 1.0, 0.0))
            matrix.rotate(-Math.toRadians(rotX), vec(1.0, 0.0, 0.0))
            matrix.scale(vec(1.0 / zoom, -1.0 / zoom, 1.0 / zoom))
    ...
    fun preDraw(mousePos: Vec2d) {
        ...
        run {
            ...
            val LIGHT0_POS = vec(0.0, 1.0, 0.1).normalize()//(new vec(0.20000000298023224D, 1.0D, -0.699999988079071D)).normalize();
            val LIGHT1_POS = vec(0.0, 1.0, -0.1).normalize()//(new vec(-0.20000000298023224D, 1.0D, 0.699999988079071D)).normalize();
