...
object Raycast {
    ...
    fun cast(entity: Entity?, dir: Vec3d, distance: Double): RayTraceResult? {
        if (entity != null) {
            ...
            var vec: Vec3d? = null
            val list = entity.worldObj.getEntitiesInAABBexcluding(entity, entity.entityBoundingBox.addCoord(dir.xCoord * distance, dir.yCoord * distance, dir.zCoord * distance).expand(1.0, 1.0, 1.0), Predicates.and(Predicate<Entity> { apply -> apply != null && (apply.canBeCollidedWith() || apply is EntityItem) }, EntitySelectors.NOT_SPECTATING))
