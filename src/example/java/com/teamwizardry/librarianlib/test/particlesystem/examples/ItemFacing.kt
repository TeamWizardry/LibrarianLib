...
object ItemFacing: ParticleExample {
    override fun update(origin: Vec3d) {
        // details here aren't important. Just generate a list of points
        for(i in 0..2) {
            ItemFacingSystem.spawn(10.0,
                ThreadLocalRandom.current().nextDouble(1/16.0, 8/16.0),
                origin + randomNormal(),
                origin,
                if(ThreadLocalRandom.current().nextBoolean()) vec(1, 1, 1) else vec(Double.NaN, Double.NaN, Double.NaN)
            )
        }
    }
}

object ItemFacingSystem: ParticleSystem() {
    ...
    fun spawn(lifetime: Double, size: Double, pos: Vec3d, origin: Vec3d, facing: Vec3d) {
